...
def mitre_lab_17_api(request):
    if request.method == "POST":
        ...
        # Validate the IP address or domain name
        if not re.match(r"^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$", ip) and not re.match(r"^[a-zA-Z0-9.-]+$", ip):
            return JsonResponse({'error': 'Invalid IP address or domain name'}, status=400)
        command = ["nmap", ip]
